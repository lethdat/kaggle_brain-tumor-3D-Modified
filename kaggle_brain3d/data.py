...
class BrainScansDataset(Dataset):
    ...
    def __init__(
        ...
        scan_types: Union[str, Sequence[str]] = ("FLAIR","T1wCE"),
        ...
        if isinstance(df_table, pd.DataFrame):
            assert all(c in df_table.columns for c in ["PatientID", "ClassLabel"])
            self.table = df_table
        ...
        for _, row in self.table.iterrows():
            ...
            self.images += imgs
            self.labels += [row["ClassLabel"]] * len(imgs)
...
class BrainScansDM(LightningDataModule):
    ...
    def __init__(
        ...
        scan_types: Sequence[str] = ("FLAIR","T1wCE"),
    ...
    def setup(self, *_, **__) -> None:
        ...
        self.test_table = [dict(PatientID=n, ClassLabel=None) for n in ls_cases]
